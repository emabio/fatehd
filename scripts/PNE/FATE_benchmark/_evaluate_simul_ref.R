...
## Retrieve input args ---------------------------------------------------------
# tests ------
setwd("/home/georgeda/Work/FATEHD/benchmarking/fatehd_params_test/FATE_newHS/SIMUL_6STRATA/")
path.to.dat <- "/home/georgeda/Work/FATEHD/benchmarking/fatehd_params_test/grid/best_soil_params_test/fhdbsoilpt" 
file_name <- "PARAM_SIMUL/paramSimul_1.txt"
nbCores <- 1 
maps <- FALSE 
...
test.simul <- file.path("outputsTables")
test.array.PFG.files <- list.files(test.simul, paste0("arrayPFG.*_rep", simul.id), full.names = TRUE)
test.times <- sort(as.numeric(sub("_.*$", "", sub("^.*_year", "", test.array.PFG.files))))
...
for(yr_ in dimnames(test.array.PFG)[[3]]){
  cat("\t", yr_)
  test.array.PFG[, , yr_] <- as.matrix(get(load(grep(paste0("_", yr_, "_rep", simul.id), test.array.PFG.files, value = TRUE))))
}
...
mask.ref <- raster(sub("SIMUL_6STRATA/", "", ref.mask.path), crs = CRS(projETRS89))
